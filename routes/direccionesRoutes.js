...
router.get('/', verifyToken, async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT d.id, d.alias, d.calle_numero, d.colonia, d.referencias, d.telefono_contacto, d.created_at,
...
router.post('/', verifyToken, async (req, res) => {
  try {
    const { alias, calle_numero, colonia, referencias, telefono_contacto } = req.body;
    ...
    const result = await pool.query(`
      INSERT INTO core.tbldirecciones (usuario_id, alias, calle_numero, colonia, referencias, telefono_contacto, created_at, updated_at)
      VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
      RETURNING id, alias, calle_numero, colonia, referencias, telefono_contacto, created_at
    `, [req.user.userId, alias.trim(), calle_numero.trim(), colonia.trim(), referencias || null, telefono_contacto || null]);
  ...
  try {
    ...
    const result = await pool.query(`
      ...
      SET alias = COALESCE($1, alias),
          ...
          updated_at = NOW()
      WHERE id = $6 AND usuario_id = $7 AND activa = TRUE
      RETURNING id, alias, calle_numero, colonia, referencias, telefono_contacto
    `, [alias, calle_numero, colonia, referencias, telefono_contacto, req.params.id, req.user.userId]);
